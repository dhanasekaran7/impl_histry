# ==================== scripts/install_option_fix.py ====================
"""
Script to automatically apply the option chain manager fix
"""
...
def backup_and_replace_files():
    """Backup original files and apply fixes"""
    
    project_root = Path(__file__).parent.parent
    
    # File paths
    option_manager_path = project_root / "src" / "options" / "option_chain_manager.py"
    
    print("🔧 Installing Option Chain Manager Fix")
    print("=" * 50)
    
    # Backup original file
    if option_manager_path.exists():
        backup_path = option_manager_path.with_suffix('.py.backup')
        shutil.copy2(option_manager_path, backup_path)
        print(f"✅ Backed up original to: {backup_path}")
    
    # The fixed option chain manager code
    fixed_option_manager_code = '''# ==================== src/options/option_chain_manager.py (FIXED VERSION) ====================
"""
...
'''
    ...
    with open(option_manager_path, 'w', encoding='utf-8') as f:
        f.write(fixed_option_manager_code)
    
    print(f"✅ Applied fix to: {option_manager_path}")
    
    print("\n📋 Fix Summary:")
    print("✅ Robust spot price fetching with multiple fallback methods")
    print("✅ Exact logic copied from your working test")
    print("✅ Better error handling and logging")
    print("✅ Caching mechanism for spot prices")
    
    print(f"\n🔧 Next steps:")
    print(f"1. Run: python scripts/test_option_chain_fix.py")
    print(f"2. If test passes, run your main.py")
    print(f"3. Monitor logs for 'Got NIFTY spot price' messages")
